---
title: Breaking (!) API changes for v2 API, July 25 to August 1
authors:
  - machine
tags:
  - apiv2
  - breaking
  - Marketplace
  - Mail
  - Notification
  - Project

---

import OperationHint from "@site/src/components/OperationHint";
import OperationLink from "@site/src/components/OperationLink";

During the week of July 25 to August 1, 2025, the mittwald API introduced several updates, including the addition of new request properties, changes in property types, the removal of deprecated properties, and the introduction of new endpoints for managing mail address backups.

{/* truncate */}



:::caution Breaking changes

This document contains changes that can under certain circumstances be considered breaking. Please review the changes carefully.

While we generally strive to avoid breaking changes in accordance with our [API stability guarantees](../../../../../docs/v2/api/stability), we occasionally might perform changes that would be considered breaking in the strictest sense of the term, but we do not consider as breaking in a practical sense. We will always provide ample notice and documentation for such changes.

:::



## Summary


- In the `PATCH` operation for the path `/v2/contributors/{contributorId}/extensions/{extensionId}`, the following changes were made:
  - The request property `detailedDescriptions` now includes `#/components/schemas/de.mittwald.v1.marketplace.DetailedDescriptions` in its `allOf` list.
  - The request property `webhookUrls` now includes `#/components/schemas/de.mittwald.v1.marketplace.WebhookUrls` in its `allOf` list.
  - The type/format of the request property `detailedDescriptions` changed from 'object' to an unspecified type.
  - The type/format of the request property `webhookUrls` changed from 'object' to an unspecified type.
  - The request property `detailedDescriptions/de` was removed.
  - The request property `detailedDescriptions/en` was removed.
  - The request property `webhookUrls/extensionAddedToContext` was removed.
  - The request property `webhookUrls/extensionInstanceRemovedFromContext` was removed.
  - The request property `webhookUrls/extensionInstanceSecretRotated` was removed.
  - The request property `webhookUrls/extensionInstanceUpdated` was removed.
  - The request property `detailedDescriptions` became nullable.
  - The request property `externalFrontends` became nullable.
  - The request property `frontendFragments` became nullable.
  - The request property `webhookUrls` became nullable.
  - Note: A breaking change has occurred due to the changes in the request properties.

- In the `GET` operation for the path `/v2/customers/{customerId}/extensions/{extensionId}`, the response property `contributorId` became required for the status '200'.

- In the `GET` operation for the path `/v2/extension-instances`, the response property `/items/contributorId` became required for the status '200'.

- In the `GET` operation for the path `/v2/extension-instances/{extensionInstanceId}`, the response property `contributorId` became required for the status '200'.

- In the `GET` operation for the path `/v2/extensions`, a new optional request parameter `includeDeprecated` was added.

- In the `GET` operation for the path `/v2/mail-addresses/{mailAddressId}/backups`, an endpoint was added to list backups belonging to a MailAddress.

- In the `POST` operation for the path `/v2/mail-addresses/{mailAddressId}/backups/{backupId}/recovery`, an endpoint was added to recover emails for a MailAddress from a backup.

- In the `GET` operation for the path `/v2/notifications`, a new optional request parameter `severity` was added.

- In the `GET` operation for the path `/v2/projects/{projectId}/extensions/{extensionId}`, the response property `contributorId` became required for the status '200'.

- In the `GET` operation for the path `/v2/projects/{projectId}/storage-space-statistics`, an optional property `meta/lastExceedanceLimitInBytes` was added to the response with the '200' status.

- In the `GET` operation for the path `/v2/servers/{serverId}/storage-space-statistics`, an optional property `meta/lastExceedanceLimitInBytes` was added to the response with the '200' status.

_Disclaimer: This summary is AI-generated. If you find any discrepancies, please refer to the detailed changes below._


## Detailed changes



### Changes in "Patch Extension"


- ⚠️ **Breaking:** added '#/components/schemas/de.mittwald.v1.marketplace.DetailedDescriptions' to the 'detailedDescriptions' request property 'allOf' list

- ⚠️ **Breaking:** added '#/components/schemas/de.mittwald.v1.marketplace.WebhookUrls' to the 'webhookUrls' request property 'allOf' list

- ⚠️ **Breaking:** the 'detailedDescriptions' request property type/format changed from 'object'/'' to ''/''

- ⚠️ **Breaking:** the 'webhookUrls' request property type/format changed from 'object'/'' to ''/''

- removed the request property 'detailedDescriptions/de'

- removed the request property 'detailedDescriptions/en'

- removed the request property 'webhookUrls/extensionAddedToContext'

- removed the request property 'webhookUrls/extensionInstanceRemovedFromContext'

- removed the request property 'webhookUrls/extensionInstanceSecretRotated'

- removed the request property 'webhookUrls/extensionInstanceUpdated'

- the request property 'detailedDescriptions' became nullable

- the request property 'externalFrontends' became nullable

- the request property 'frontendFragments' became nullable

- the request property 'webhookUrls' became nullable


For details, refer to the <OperationLink operation="extension-patch-extension" apiVersion="v2" /> endpoint.



### Changes in "Get the extension instance of a specific customer and extension, if existing"


- the response property 'contributorId' became required for the status '200'


For details, refer to the <OperationLink operation="extension-get-extension-instance-for-customer" apiVersion="v2" /> endpoint.



### Changes in "List extension instances"


- the response property '/items/contributorId' became required for the status '200'


For details, refer to the <OperationLink operation="extension-list-extension-instances" apiVersion="v2" /> endpoint.



### Changes in "Get an extension instance"


- the response property 'contributorId' became required for the status '200'


For details, refer to the <OperationLink operation="extension-get-extension-instance" apiVersion="v2" /> endpoint.



### Changes in "List Extensions"


- added the new optional 'query' request parameter 'includeDeprecated'


For details, refer to the <OperationLink operation="extension-list-extensions" apiVersion="v2" /> endpoint.



### Changes in "List backups belonging to a mail address"


- endpoint added


For details, refer to the <OperationLink operation="mail-list-backups-for-mail-address" apiVersion="v2" /> endpoint.



### Changes in "Recover emails for a mail address from a backup"


- endpoint added


For details, refer to the <OperationLink operation="mail-recover-mail-address-emails" apiVersion="v2" /> endpoint.



### Changes in "List all unread notifications"


- added the new optional 'query' request parameter 'severity'


For details, refer to the <OperationLink operation="notifications-list-notifications" apiVersion="v2" /> endpoint.



### Changes in "Get the extension instance of a specific project and extension, if existing"


- the response property 'contributorId' became required for the status '200'


For details, refer to the <OperationLink operation="extension-get-extension-instance-for-project" apiVersion="v2" /> endpoint.



### Changes in "Get storage space Statistics belonging to a Project"


- added the optional property 'meta/lastExceedanceLimitInBytes' to the response with the '200' status


For details, refer to the <OperationLink operation="storagespace-get-project-statistics" apiVersion="v2" /> endpoint.



### Changes in "Get storage space Statistics belonging to a Server"


- added the optional property 'meta/lastExceedanceLimitInBytes' to the response with the '200' status


For details, refer to the <OperationLink operation="storagespace-get-server-statistics" apiVersion="v2" /> endpoint.




## Client package releases


### mittwald JavaScript SDK Release 4.193.0

The mittwald JavaScript SDK has been updated to version 4.193.0. This release includes an update to the generated client, enhancing the SDK's functionality. For more details, you can view the release on [GitHub](https://github.com/mittwald/api-client-js/releases/tag/4.193.0).

### mittwald JavaScript SDK Release 4.192.0

The mittwald JavaScript SDK has been updated to version 4.192.0. This release includes an update to the generated client, enhancing the SDK's functionality. For more details, you can view the release on GitHub [here](https://github.com/mittwald/api-client-js/releases/tag/4.192.0).

### mittwald JavaScript SDK Release 4.191.0

The mittwald JavaScript SDK has been updated to version 4.191.0. This release includes an update to the generated client, enhancing the SDK's functionality. For more details, you can view the release on GitHub: [mittwald JavaScript SDK 4.191.0](https://github.com/mittwald/api-client-js/releases/tag/4.191.0).

