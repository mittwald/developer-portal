...
- **Extension ID**:  
  Die Extension ID kannst du im Tab **Entwicklung** deiner Extension im mStudio kopieren.

- **Scopes**:  
  Im Bereich **Berechtigungen** im mStudio findest du die ausgewählten Scopes.  
  Für jeden ausgewählten Scope musst du im `scopes`-Feld deines Request-Bodys einen Eintrag im folgenden Format hinzufügen:\
  `"area:(read|write|delete)"`.

![Extension permissions section](../../../../../../resources/contributing/extensions/extension-berechtigungen.jpg)

:::tip
Wenn du eine Extension Instance über das mStudio erstellst, musst du dich nicht darum kümmern, den richtigen context zu setzen oder die richtigen Scopes auszuwählen.
:::
