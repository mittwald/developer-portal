...
> :warning: Beachte, dass **jede** Änderung der Preisdetails (inklusive dem Hinzufügen von Varianten) zu einer 30-tägigen Sperre für erneute Bearbeitungen führt

![Extension Preisdetail Bereich](../../../../../../resources/contributing/extensions/PricingSection-mStudio.png)
...
- Wenn die Extension zuvor einen Einzelpreis hatte, kann der **erste angelegte Preis** als Variante mit dem **"default" Key** weiterverwendet werden. Diese Variante benötigt ab sofort ebenfalls eine Beschreibng ihrer Leistungsmerkmale.
...
Wenn Preis- oder Leistungsumfangsänderungen vorgenommen werden, sorgt das System dafür, dass bestehende Kunden informiert werden und den neuen Bedingungen **zustimmen** oder **ablehnen** müssen. Lehnt der Kunde ab oder reagiert 30 Tage nicht, wird seine Instanz entfernt.
...
- Die vom Nutzer gewählte **Variante** geändert wird. Dies betrifft nur Ämnderungen am Preis, oder dem Leistungsumfang. Sollten die Leistungsmerkmale kosmetisch verändert werden (Rechtschreibfehler etc.) hat der Contributor die Wahl, keine Leistungsumfangsänderung anzukündigen.
...
Jede Extension erhält Benachrichtigungen vom mStudio über die [**Lifecycle Webhooks**](https://developer.mittwald.de/docs/v2/contribution/reference/webhooks/), falls diese eingerichtet wurden.  
...
> Er muss im Backend des Contributors eindeutig auf die entsprechende Variante abgebildet werden, insofern eine Unterscheidung der Varianten dort notwendig ist.
...
- Die korrekte Verarbeitung des `variantKey` im eigenen Backend, falls notwendig
...
- Teste alle Variantenwechsel und Leistungsumfänge gründlich vor der Veröffentlichung.
...
| -------------------------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------ |
| Variante wird im Backend nicht erkannt | Fehlender oder falscher `variantKey`                     | Webhook-Payload, falls in Verwendung, korrekt verarbeiten und Zuordnung prüfen |
...
Eine saubere Webhook-Verarbeitung oder die Nutzung des variantKey aus dem Session Token, und konsistente Variantenverwaltung sind dabei entscheidend für korrekte Abrechnung und Kundenerfahrung.
