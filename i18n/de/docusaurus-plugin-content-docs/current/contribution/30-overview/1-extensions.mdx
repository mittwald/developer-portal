...
Aufgrund von api-Stabilitätsgarantien ist das Feld `state` deprecated.
Die Felder `disabled`, `published`, `verifcatonRequested`, `deprecated`, `functional` und `verified` der `OwnExtension`,
wie sie unter <OperationLink operation="extension-get-own-extension" />; zu finden sind, zeigen den Zustand der Extension an.

- `disbled`: Die Erweiterung ist deaktiviert. Es können keine neuen **ExtensionInstances** erstellt werden.
- `published`: Jeder kann nun die Erweiterung zu seinem eigenen **Kontext** hinzufügen.
- `verifcatonRequested`: mittwald verifiziert die **Erweiterung**.
- `deprecated`: Die **Erweiterung** ist durch den **Beitragenden** veraltet.
- `funcional`: Die **Erweiterung** funktioniert wie erwartet.
- `verified`: Die Erweiterung wurde von mittwald verifiziert und kann nun veröffentlicht werden.
...
A --> B
C --> D
C --> B: Extension Kaufprozess abgeschlossen
B --> D: Durch einen Nutzer entfernt
B --> C: Extension wurde kostenpflichtig
```
