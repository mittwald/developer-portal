...
Die Scopes, der Kontext sowie Support-Kontaktdaten müssen ebenfalls gesetzt sein.
Wenn die Extension nicht kostenlos angeboten werden soll, werden zudem Webhook-URLs benötigt.
...
Dies kann nützlich sein, um Wartungen durchzuführen.

Die Extension kann ohne erneute Verifizierung wieder veröffentlicht werden.

### Änderungen an der Extension

Änderungen an den Scopes und den Webhook-URLs bedürfen einer erneuten Verifizierung durch mittwald.
