...
Nach der Registrierung einer Extension kann eine Verifizierung beantragt werden.
Dazu müssen folgende Informationen bereitgestellt werden:

- Ein eindeutiger Name für die Extension.
- Definierte Scopes und Context.
- Support-Kontaktdaten.
- Falls die Extension kostenpflichtig ist, müssen auch webhookUrls angegeben werden.

Beitragende haben jederzeit die Möglichkeit, den Verifizierungsprozess abzubrechen.
Nach der Einreichung wird die Extension von mittwald überprüft und verifiziert.
Wenn keine Rückmeldungen erforderlich sind und alle Anforderungen erfüllt wurden, kann die Extension veröffentlicht werden.
Nur die Contributoren haben die Berechtigung, eine Extension zu veröffentlichen.

## Nach der Veröffentlichung

<OperationLink operation="extension-set-extension-published-state" />

Nach der Veröffentlichung kann eine Extension von dem Contributor zurückgezogen werden.
Dies verhindert die Erstellung neuer Extension Instances und deaktiviert bestehende Extension Instances, was für Wartungszwecke nützlich sein kann.
Die Extension kann später erneut veröffentlicht werden, ohne eine erneute Verifizierung zu benötigen.
...
- Der Context einer Extension kann nur geändert werden, bevor die erste Verifizierungsanfrage gestellt wurde.
- Während der Entwicklung kann der Context frei geändert werden,
  allerdings werden dabei alle bestehenden Extension Instaces entfernt.

### Hinzufügen von Erweiterungs Instanzen zu einem Kontext

Um eine Extension Instance zu einem Context hinzuzufügen, muss der Context zunächst gesetzt sein.

- Vor der Veröffentlichung kann nur Contributor Extension Instances innerhalb der eigenen Organisation oder Projekte erstellen.
- Nach der Veröffentlichung kann jeder Extension Instances erstellen.

### Löschen einer Extension

Um eine Extension geordnet zu entfernen, sollte ein Deprecation-Prozess definiert werden.
Dieser Prozess umfasst:

- Die Festlegung eines Abschaltdatums.
- Die Definition eines Nachfolgers, falls zutreffend.

Extension werden asynchron gelöscht, da zuvor alle zugehörigen Extension Instances entfernt werden müssen.
