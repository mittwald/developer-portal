...
Sende eine `GET`-Anfrage an den Endpunkt `/v2/redis-versions`, um eine Liste der verfügbaren Redis-Versionen zu erhalten (<OperationLink tag="database" operation="database-list-redis-versions">Dokumentation für den `database-list-redis-versions`-Endpunkt</OperationLink>).
