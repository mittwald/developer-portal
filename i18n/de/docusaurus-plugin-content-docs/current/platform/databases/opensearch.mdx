...
## Was ist OpenSearch? {#what-is-opensearch}
...
## Eine OpenSearch-Datenbank erstellen {#creating-an-opensearch-database}
...
### Verwendung von Terraform (Empfohlen) {#using-terraform-recommended}
...
### Verwendung der mStudio-Benutzeroberfläche {#using-the-mstudio-ui}
...
### Verwendung der CLI mit `mw container run` {#using-the-cli-with-mw-container-run}
...
### Verwendung der CLI mit `mw stack deploy` {#using-the-cli-with-mw-stack-deploy}
...
### Zugriff auf deinen Container innerhalb deiner Hosting-Umgebung {#accessing-your-container}
...
## Einrichtung gängiger Anwendungen {#setting-up-common-applications}
...
TYPO3 unterstützt OpenSearch nicht nativ, daher ist eine Erweiterung erforderlich. Eine mögliche Integration wäre [`pluswerk/elasticsearch`](https://github.com/pluswerk/elasticsearch). Alternativ empfehlen wir die Verwendung von [Solr](../solr), welches gut von TYPO3 unterstützt wird.
