...
Um die Authentifizierung für die Registry einfach zu konfigurieren, verwenden wir das [`mittwald/registry`-Image](https://hub.docker.com/repository/docker/mittwald/registry/general) anstelle des regulären `registry`-Images von Docker Hub. Dieses Image basiert auf dem offiziellen `registry`-Image, enthält aber zusätzliche Funktionen und Konfigurationsoptionen für die Authentifizierung.
...
<Tabs groupId="access-mode">
  <TabItem value="gui" label="mStudio UI">
    ...
    4. Konfiguriere den Container nach Bedarf und verwende das Image `mittwald/registry:3`.
    5. Richte Umgebungsvariablen nach deinen Wünschen ein, siehe die obige Anleitung für Details.
    6. Um die Authentifizierung (für private Images) zu konfigurieren, gib zusätzlich eine `REGISTRY_USER`- und eine `REGISTRY_PASSWORD`-Umgebungsvariable mit sicheren Zugangsdaten deiner Wahl an.
  ...
  <TabItem value="cli" label="CLI">
    ...
    user@local $ mw container run \
      ...
      --env "REGISTRY_USER=<dein-benutzername>" \
      --env "REGISTRY_PASSWORD=<dein-sicheres-passwort>" \
      --create-volumes \
      mittwald/registry:3
    ```

    Ersetze `<dein-benutzername>` und `<dein-sicheres-passwort>` durch sichere Zugangsdaten deiner Wahl. Beachte, dass über die Kommandozeile übergebene Passwörter in der Shell-History gespeichert werden können.
...
Wenn eine Anmeldung über Zugangsdaten gewünscht ist, denke daran, die Umgebungsvariablen `REGISTRY_USER` und `REGISTRY_PASSWORD` mit sicheren Zugangsdaten deiner Wahl anzugeben.
:::
...
Wenn du die Authentifizierung über `REGISTRY_USER` und `REGISTRY_PASSWORD` konfiguriert hast, musst du dich anmelden, bevor du Images pushen kannst:
...
### Aktualisieren eines veränderlichen Tags {#updating-a-mutable-tag}
...
Bei veränderlichen Tags (wie `latest`) wird das häufig mit einem wiederkehrenden Update-Zeitplan kombiniert (siehe [Wiederkehrende Container-Updates planen](#update-schedule)), damit der Stack regelmäßig die neueste Image-Version abruft und anwendet.
...
Für Datenbanken, die in Containern laufen, beschreibt die Anleitung [Datenbanken in Containern sichern](/docs/v2/guides/operations/database-container-backups), wie du konsistente SQL-Dumps in ein Volume einrichtest, sodass das Projekt-Backup sie erfasst.
...
## Wiederkehrende Container-Updates planen {#update-schedule}

Wenn deine Deployment-Strategie veränderliche Tags verwendet, kannst du einen Zeitplan für Stack-Updates festlegen. So kann der Stack regelmäßig neuere Image-Versionen abrufen, ohne dass du den Update-Befehl jedes Mal manuell ausführen musst.

Du kannst den Update-Zeitplan in der mStudio UI, per CLI oder per API mit der <OperationLink operation="container-set-stack-update-schedule" apiVersion="v2" />-Operation konfigurieren.

Verwende dafür den Befehl `mw stack set-update-schedule`:

```shellsession
user@local $ mw stack set-update-schedule <stack-id> <schedule> [--timezone=<timezone>]
```

Der Wert `<schedule>` ist ein Cron-Ausdruck. Um das Update zum Beispiel täglich um 03:00 Uhr UTC auszuführen:

```shellsession
user@local $ mw stack set-update-schedule 11111111-2222-3333-4444-555555555555 "0 3 * * *"
```

Verwende als `<stack-id>` die vollständige Stack-ID im UUID-Format.

Nutze das besonders dann, wenn deine Container veränderliche Tags wie `latest` verwenden, damit neue Image-Versionen automatisch gemäß deinem Zeitplan angewendet werden.
