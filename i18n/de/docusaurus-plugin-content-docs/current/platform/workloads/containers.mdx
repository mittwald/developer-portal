...
Dies erstellt die `htpasswd`-Anmeldedatei, die wir jetzt übertragen müssen. Denke daran, `exampleuser` und `examplepassword` durch geeignete Zugangsdaten zu ersetzen.
...
(Re-)Erstelle den Container mit den festgelegten Volumes und Umgebungsvariablen.
