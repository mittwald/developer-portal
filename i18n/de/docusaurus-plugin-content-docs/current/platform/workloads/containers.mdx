...
Bei veränderlichen Tags (wie `latest`) wird das häufig mit einem wiederkehrenden Update-Zeitplan kombiniert, damit der Stack regelmäßig die neueste Image-Version abruft und anwendet.
...
Verwende als `<stack-id>` die kurze Stack-ID (zum Beispiel `st-abcde`).
