...
Da die Bereitstellung deiner Anwendung für den Produktivbetrieb optimiert ist, wird die Plattform deine Anwendung **NICHT** automatisch neu starten, wenn du den Code änderst. Du musst deine Anwendung nach der Bereitstellung manuell neu starten. Sieh dir den Abschnitt zur [Verwaltung des Prozesslebenszyklus](#process-lc) an, um mehr darüber zu erfahren.

:::

## Deine Anwendung über HTTP zugänglich machen

Lies unseren Leitfaden zu [Webservern](../webservers#http-access), um zu erfahren, wie du deinen PHP-Worker über HTTP erreichbar machst.

## Verwaltung des Prozesslebenszyklus {#process-lc}

Lies unseren Leitfaden zu [Webservern](../webservers#process-lc), um zu erfahren, wie du den Prozesslebenszyklus deines PHP-Workers verwalten kannst.
...
```shellsession
user@local $ mw app create php-worker --wait --entrypoint "php server.php"
```
...
```shellsession
user@ssh $ composer init --require=react/http --require=react/socket
```
...
```shellsession
user@local $ mw app create php-worker --wait --entrypoint "bash -c 'php artisan serve --port $PORT'"
```
...
```shellsession
user@local $ mw app create php-worker --wait --entrypoint "php bin/console messenger:consume async"
```
...
### Mehrere Worker-Instanzen ausführen (mit PM2) {#running-multiple-worker-instances}
...
```shellsession
user@local $ mw app dependency update <app-id> --set node=~22.0
```

Dann installiere PM2 über NPM:

```shellsession
user@ssh $ npm install -g pm2
```
...
```shellsession
user@local $ mw app update <app-id> --entrypoint "pm2 start ecosystem.config.js --no-daemon"
```

### Mehrere Worker-Instanzen ausführen (mit Supervisord) {#running-multiple-worker-instances-supervisord}

[Supervisord](https://supervisord.org/index.html) ist ein weiterer beliebter Prozessmanager, der verwendet werden kann, um mehrere Worker-Instanzen innerhalb derselben Worker-App auszuführen.

Um Supervisord zu verwenden, füge Python als Abhängigkeit zu deiner PHP-Worker-App hinzu:

```shellsession
user@local $ mw app dependency update <app-id> --set python=~3.13
```

Dann installiere Supervisord über pip:

```shellsession
user@ssh $ pip install supervisor
```

Erstelle im Anwendungsverzeichnis eine Konfigurationsdatei `supervisord.conf`:
...
directory=/html/actual-installation-directory
```

Stelle sicher, dass `supervisord` als Entrypoint für deine PHP-Worker-App verwendet wird:

```shellsession
user@local $ mw app update <app-id> --entrypoint "supervisord"
```

Danach kannst du auch den Befehl `supervisorctl` verwenden, um deine Worker-Prozesse zu verwalten:

```shellsession
user@ssh $ supervisorctl status

# alternativ von deinem lokalen Rechner:
user@local $ mw ssh <app-id> "supervisorctl status"
```

[deployment]: /docs/v2/guides/deployment/
