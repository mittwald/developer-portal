...
Schickst du das an `/v1/responses` statt an `/v1/chat/completions`? `chat_template_kwargs` gehört nicht zur Responses-API und wird dort entfernt, solange es nicht in `allowed_openai_params` genannt ist — siehe [Die Denkphase abschalten](../../api-endpoints/supported-endpoints#responses-thinking), dort steht auch das Feld `reasoning.effort` des Endpunkts.
