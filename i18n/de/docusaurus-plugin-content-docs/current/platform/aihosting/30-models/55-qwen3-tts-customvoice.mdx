...
- Streaming, entweder als rohe Audio-Chunks oder als `speech.audio.delta`-Events im OpenAI-Format, womit das Audio anfängt, bevor die Generierung durch ist
...
:::warning[`language` will einen Sprachnamen, keinen ISO-Code]
...
Besseres Deutsch bekommst du damit nicht. Derselbe deutsche Satz, je drei Durchläufe, zurücktranskribiert: Mit `language: "German"` kamen zwei von drei Durchläufen wortgleich zurück, ohne den Parameter ebenfalls zwei von drei. Die Spracherkennung des Modells reicht also, lass `language` weg, solange du keinen Grund hast, die Sprache festzunageln.

Wenn du einen gemeinsamen Wrapper um beide Audio-Modelle baust, ist das der Parameter, über den du stolperst. [Whisper](/docs/v2/platform/aihosting/models/whisper-large-v3-turbo) will ISO-639-1-Codes wie `de`, diese Route weist sie ab und will `German`. Derselbe Wert funktioniert also auf einer Route und scheitert auf der anderen.
:::
...
Die Größen sind absichtlich auf eine Sekunde Audio normiert. Das Modell sampelt, zwei Durchläufe über denselben Text unterscheiden sich in der Länge um eine Sekunde oder mehr, und ein Vergleich der reinen Dateigrößen vergleicht dann die Längen und nicht die Formate. Normiert sind `pcm` und `wav` dasselbe: je 48,0 kB pro Sekunde, wobei `wav` einen Header von 44 Byte dazulegt.
...
| Du schickst                                            | Das Modell sagt, zwei Durchläufe je Zeile                                                                                          |
| ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
...
Deutsch reagiert also genauso deutlich wie Englisch. Eine Emotion ist damit auch auf deutschem Text ein brauchbarer Hebel.
...
Wie stark eine kurze Zeile danebengeht, ist selbst unbeständig. Nachgemessen mit anderen Sätzen, je vier Durchläufe: Eine Zeile mit 20 Zeichen kam vier von vier Mal sauber zurück, eine Zeile mit 60 Zeichen lag im Mittel bei einer Wortfehlerrate von 0,025, während eine Zeile mit 41 Zeichen im Mittel 0,375 erreichte und zweimal deutlich danebenlag. Nimm die Länge als Risikoindikator, nicht als Schwellwert, den du berechnen kannst.
:::
...
Echte Umlaute sind die bessere Schreibweise, keine Garantie. Ein kürzerer Satz mit korrekten Umlauten, 38 Zeichen, fünf Durchläufe, lag bei einer mittleren Wortfehlerrate von 0,28 mit zwei sauberen Durchläufen. Das ist der Kurztext-Effekt von weiter oben und kein Umlaut-Problem. Behalt die Umlaute und gib dem Modell genug Text.
:::
...
| ----------------------- | ------------------- | ------------ | --------------------------------------------- |
| gepuffert (Standard)    | `audio/pcm`         | 870 ms       | ein Körper                                    |
...
`stream_format` wählt die Hülle. `"audio"` liefert eine Chunked-Antwort mit rohem Audio im angeforderten Format, und das willst du, wenn du in eine Datei schreibst oder in einen Player pipest. `"sse"` liefert die Event-Hülle von OpenAI: `speech.audio.delta`-Events mit dem Audio als Base64, abgeschlossen von einem `speech.audio.done`.
