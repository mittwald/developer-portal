...
Von der falschen Seite lässt sich das gut prüfen. Deutscher Text mit `language: "English"` kommt trotzdem als korrektes Deutsch zurück. Zurücktranskribiert: „Die Lieferung kommt am Dienstagmorgen an und die Rechnung wurde bereits an ihre Adresse geschickt."

Daraus folgen zwei Dinge. Die Spracherkennung liegt ganz beim Modell, und es hat Deutsch und Englisch in jedem unserer Durchläufe richtig erkannt. Den Parameter weglassen kostet dich also nichts. Und du kannst keine Sprache erzwingen: Wird ein Text in der falschen Sprache gelesen, hilft `language` nicht, sondern nur, den Text aufzuteilen.
...
| `<laugh> The delivery arrives on Tuesday morning.`     | beide Male nur der Satz, kein Lachen                                                                                              |
...
Nimm die Auszeichnungen aus deinem Text heraus und schreib stattdessen in den Parameter `instructions`, wie die Zeile klingen soll. Ein normaler Satz reicht, höchstens 500 Zeichen. Längere Werte führen zu HTTP 500.
...
`instructions` ist in den OpenAI-SDKs ein regulärer Parameter und braucht deshalb kein `extra_body`. Lass `language` weg: Die Route ignoriert den Parameter, und englischen Text liest das Modell von selbst als Englisch.
