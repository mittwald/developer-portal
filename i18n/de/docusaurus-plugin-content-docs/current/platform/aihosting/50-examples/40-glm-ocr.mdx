...
GLM-OCR ist ein zweckoptimiertes Modell zum Verstehen von Dokumenten. Anders als allgemeine Vision-Modelle ist es ausschließlich auf das Lesen von Text ausgelegt – und damit die richtige Wahl, wenn du PDFs digitalisieren, gescannte Rechnungen parsen oder Felder aus Formularen mit hoher Genauigkeit und geringen Kosten extrahieren möchtest.

:::tip Wann GLM-OCR einsetzen?
Verwende GLM-OCR, wenn das Ziel **Textextraktion** aus einem Dokumentenformat ist (PDF, DOCX, PPTX, XLSX, HTML, Bilder). Für Aufgaben, die Verstehen und Konversation kombinieren (z. B. „Fasse diesen Vertrag zusammen und erkläre ihn"), ist ein allgemeines Vision-Modell wie [Qwen3.5-122B-A10B-FP8](/docs/v2/platform/aihosting/models/qwen3-5-122b-a10b-fp8) möglicherweise besser geeignet. Natürlich kannst du Deine Dokumente auch mit GLM-OCR vorvearbeiten und an ein anderes LLM weiterleiten.

Eine Übersicht aller unterstützten Formate, Limits und Inferenzparameter enthält die [GLM-OCR Modellreferenz](/docs/v2/platform/aihosting/models/glm-ocr).
:::
...
GLM-OCR unterstützt drei Ausgabemodi, die ausschließlich über den Prompt gesteuert werden:

| Modus | Wann sinnvoll | Beispiel-Prompt |
| ----- | ------------- | --------------- |
| **Klartext** | Volltextsuche, Logging, einfache Digitalisierung | `"Extrahiere den gesamten Text aus diesem Dokument."` |
| **Markdown** | Dokumentstruktur erhalten (Überschriften, Listen, Tabellen) für Anzeige oder RAG | `"Extrahiere den Text und formatiere ihn als Markdown. Verwende # für Überschriften und - für Listen."` |
| **JSON** | Bestimmte benannte Felder aus Formularen, Rechnungen, Zertifikaten extrahieren | `"Extrahiere diese Felder und gib sie als JSON zurück: …"` |

:::note[Ausgabeverhalten]
**JSON-Modus umschließt die Ausgabe immer mit Markdown-Code-Blöcken** (` ```json … ``` `) – diese müssen vor dem Aufruf von `json.loads()` entfernt werden. Das Stripping-Muster zeigt das [KIE-Beispiel](#strukturierte-daten-aus-rechnungen-extrahieren) weiter unten.

**Markdown-Modus benötigt konkrete Formatierungsanweisungen.** Ein allgemeines „Wandle in Markdown um" reicht möglicherweise nicht aus, um Überschriften-Syntax zuverlässig zu erzeugen. Immer `# für Hauptüberschriften, ## für Unterüberschriften, - für Listen` angeben.

**HTML-Quelle → HTML-Tabellen.** Enthält die Eingabe eine HTML-Datei mit einer `<table>`, gibt das Modell diese tendenziell als HTML aus – auch wenn nach Markdown-Tabellen gefragt wird. Für saubere Markdown-Tabellenausgabe eine PDF- oder Bildquelle verwenden.
:::
...
:::note[Datenverarbeitung und Datenschutz]
**Es werden keine Dokumentdaten auf Datenträgern gespeichert.** Der Proxy rendert Dokumente ausschließlich im Arbeitsspeicher (RAM) zu PNG-Bildern und leitet sie direkt an das Modell weiter – nach Auslieferung der Antwort werden keine Daten aufbewahrt. Externe URLs in HTML-, CSS- und SVG-Inhalten werden explizit gesperrt, um SSRF-Angriffe zu verhindern. Dokumentinhalte werden ausschließlich innerhalb der mittwald-Infrastruktur verarbeitet und nicht an Dritte übermittelt.
:::
...
:::note[Seitenlimit]
Pro Anfrage werden bis zu **30 Seiten** verarbeitet. Bei größeren Dokumenten gibt die API HTTP 413 zurück – siehe [Große PDFs in Batches verarbeiten](#grosse-pdfs-in-batches-verarbeiten) weiter unten.
:::
...
:::note[XLSX-Ausgabe]
Bei Excel-Dateien gibt das Modell den **Tabellenblatt-Namen als erste Zeile** aus, gefolgt vom Zelleninhalt. Beim programmatischen Weiterverarbeiten diese erste Zeile herausfiltern oder berücksichtigen.

Office-Dateien müssen gültige Open-XML-Dokumente sein. Beschädigte oder selbst erstellte ZIP-Strukturen führen zu HTTP 400.
:::
