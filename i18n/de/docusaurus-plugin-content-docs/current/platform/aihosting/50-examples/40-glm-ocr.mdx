...
for frage in fragen:
    ...
    kontext_abschnitte = neu_sortieren(frage, kandidaten, top_k=3)
    print(f"\nF: {frage}")
...
Die meisten Dokumente brauchen das Bild nicht – reiner OCR-Text sortiert genauso gut und ist günstiger zu senden. Ohne `image_url`-Content auszukommen und stattdessen reine Strings als `documents` zu übergeben, ist genau das, was `neu_sortieren()` in der [vollständigen Pipeline](#vollstaendige-pipeline) oben bereits tut. Greife nur dann zu `neu_sortieren_multimodal()`, wenn das visuelle Layout – ein Stempel, ein Diagramm, eine markierte Zahl – für die Antwort wirklich eine Rolle spielt.
