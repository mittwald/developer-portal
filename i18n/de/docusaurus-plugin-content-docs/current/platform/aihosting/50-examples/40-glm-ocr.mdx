...
Unterstützte Formate, Limits, Ausgabemodi, Inferenzparameter und JavaScript-/PHP-Codebeispiele stehen in der [GLM-OCR Modellreferenz](/docs/v2/platform/aihosting/models/glm-ocr).
:::
...
## Vollständige Pipeline: OCR → Einbettung → Suche → Antwort {#full-pipeline}

Kombiniere vier mittwald AI-Hosting-Modelle zu einer vollständigen Dokumenten-Frage-Antwort-Pipeline:
...
2. **[Qwen3-Embedding-8B](/docs/v2/platform/aihosting/models/qwen3-embedding-8b)** wandelt Textabschnitte in Vektoren für einen schnellen ersten Retrieval-Durchgang um
3. **[Qwen3-VL-Reranker-2B](/docs/v2/platform/aihosting/models/qwen3-vl-reranker-2b)** bewertet die gefundenen Kandidaten erneut anhand der vollständigen Frage und grenzt sie auf die relevantesten wenigen ein
4. **[Qwen3.5-122B-A10B-FP8](/docs/v2/platform/aihosting/models/qwen3-5-122b-a10b-fp8)** beantwortet Fragen in natürlicher Sprache anhand des neu sortierten Kontexts
...
### Retrieval-Ergebnisse neu sortieren {#reranking}

Embedding-Ähnlichkeit ist schnell – ein Skalarprodukt pro Kandidat – aber ungenau: Sie bewertet Frage und Abschnitt unabhängig voneinander und kann deshalb einen Abschnitt übersehen, der die Frage mit anderen Worten beantwortet, als die Frage selbst verwendet. Ein **Reranker** betrachtet Frage und einen Abschnitt gemeinsam, als Paar, und bewertet, wie gut genau dieses Paar zusammenpasst – anschließend sortiert er die engere Auswahl nach diesem Wert neu. Pro Kandidat ist das langsamer als die Vektorsuche, deshalb läuft der Reranker nur über die Handvoll Abschnitte, die `abrufen()` bereits vorausgewählt hat, nie über den gesamten Index.

`Qwen3-VL-Reranker-2B` läuft über den `/v1/rerank`-Endpunkt statt über `/v1/chat/completions`:

```python
# ── Reranking: Präzisionsdurchgang über die abgerufenen Kandidaten ────────────
...
def neu_sortieren(
    ...
    payload = {"model": "Qwen3-VL-Reranker-2B", "query": frage, "documents": abschnitte}
    if anweisung:
...
Rufe zunächst einen größeren Kandidatenpool aus dem Vektorindex ab und lass den Reranker ihn anschließend vor der Antwort eingrenzen:
...
for frage in fragen:
    ...
    kontext_abschnitte = neu_sortieren(
        ...
        kandidaten,
        top_k=3,
        anweisung="Given a question, find the document passage that best answers it.",
    )
    print(f"\nF: {frage}")
...
:::tip Skalierung
Für den Produktionseinsatz empfiehlt es sich, die In-Memory-Kosinus-Suche durch **pgvector** zu ersetzen – eine PostgreSQL-Erweiterung für Vektorsimilarität, die auf mittwald Container Hosting zusammen mit n8n läuft. Siehe den [Abschnitt zum vollständigen RAG mit n8n](#n8n) weiter unten oder den [mittwald n8n-Guide](/docs/v2/guides/apps/n8n/) für einen startbereiten Docker-Compose-Stack.
:::

## Mehrere Dokumente indizieren {#multiple-documents}

Dokumente unterschiedlichen Typs lassen sich frei in einem Index mischen – der Aufbau bleibt gleich, egal wie viele Dateien es sind:

```python
dokumente = ["vertrag_a.pdf", "vertrag_b.docx", "anhang.xlsx"]
...
# Über alle Dokumente hinweg abfragen
frage = "Welche Vertragsstrafen gelten?"
[anfrage_vektor] = einbetten([frage])
kandidaten = abrufen(anfrage_vektor, index, top_k=10)
top_abschnitte = neu_sortieren(frage, kandidaten, top_k=3)
print(antworten(frage, top_abschnitte))
```

Das nutzt `dokument_ocr`, `text_aufteilen`, `einbetten`, `abrufen`, `neu_sortieren` und `antworten` genau so, wie sie oben in der Pipeline definiert sind – mehr Dokumente zu indizieren ändert nur die Größe von `dokumente`, nicht die Pipeline selbst.

## Multimodale Variante: Reranking mit Bildern {#multimodal}

Bei Dokumenten, die ursprünglich gescannte Bilder waren – `scan.jpg`, `diagramm.png` und Ähnliches aus [Text aus Bildern extrahieren](#text-aus-bildern-extrahieren) oben – trägt das Originalbild oft noch Informationen, die im OCR-Text verloren gehen: ein Stempel, ein Logo, eine markierte Summe, das Layout eines Diagramms. `Qwen3-VL-Reranker-2B` akzeptiert Dokumente, die Text und Bildinhalt kombinieren – du kannst dem Reranker also sowohl den OCR-Abschnitt als auch die Seite geben, von der er stammt.

Das funktioniert nur sauber, wenn ein Abschnitt genau einem Quellbild entspricht – ein einseitiger Scan, kein mehrseitiges PDF, das in viele Abschnitte aufgeteilt wurde. Verfolge den Quellpfad zusammen mit jedem Abschnitt beim Indizieren:
...
bild_dokumente = ["scan.jpg", "diagramm.png", "grafik.webp"]

bild_index: list[tuple[str, str]] = []  # (abschnitt_text, quellbild_pfad)
for pfad in bild_dokumente:
    text = dokument_ocr(pfad)
    for abschnitt in text_aufteilen(text):
        bild_index.append((abschnitt, pfad))
```

Erweitere `neu_sortieren()` so, dass ein Bild neben dem Text jedes Abschnitts bewertet wird – jedes Dokument wird dazu als Liste von Content-Elementen statt als reiner String übergeben:

```python
def neu_sortieren_multimodal(
    frage: str,
    abschnitte: list[str],
    bilder: list[str],
    top_k: int = 3,
    ...
    dokumente = [
        {
            "content": [
                {"type": "text", "text": abschnitt},
                {"type": "image_url", "image_url": {"url": bild_kodieren(bild_pfad)}},
            ]
        }
        for abschnitt, bild_pfad in zip(abschnitte, bilder)
    ]
    payload: dict = {"model": "Qwen3-VL-Reranker-2B", "query": frage, "documents": dokumente}
    if anweisung:
        payload["instruction"] = anweisung
    resp = requests.post(
        "https://llm.aihosting.mittwald.de/v1/rerank",
...
Rufe die Funktion mit den oben aufgebauten parallelen Abschnitts- und Bildlisten auf, anstelle von `neu_sortieren()`:

```python
abschnitte = [a for a, _ in bild_index]
bilder = [p for _, p in bild_index]
top_abschnitte = neu_sortieren_multimodal("Was zeigt die Grafik?", abschnitte, bilder, top_k=3)
```

### Nur Text als Fallback {#text-only}

Die meisten Dokumente brauchen das Bild nicht – reiner OCR-Text sortiert genauso gut und ist günstiger zu senden. Ohne `image_url`-Content auszukommen und stattdessen reine Strings als `documents` zu übergeben, ist genau das, was `neu_sortieren()` in der [vollständigen Pipeline](#full-pipeline) oben bereits tut. Greife nur dann zu `neu_sortieren_multimodal()`, wenn das visuelle Layout – ein Stempel, ein Diagramm, eine markierte Zahl – für die Antwort wirklich eine Rolle spielt.

:::tip
Schreibe den `instruction`-Wert immer auf **Englisch**, auch wenn die Anfrage in einer anderen Sprache formuliert ist. Details zu Anweisungsformat und Bildgrößen-Limits stehen auf der [Qwen3-VL-Reranker-2B Modellseite](/docs/v2/platform/aihosting/models/qwen3-vl-reranker-2b).
:::
