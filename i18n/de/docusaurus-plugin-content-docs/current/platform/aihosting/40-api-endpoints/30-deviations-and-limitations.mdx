...
- Auf `/v1/audio/speech` antwortet ein ungültiger Parameterwert mit HTTP 500 und einer allgemeinen Meldung statt mit einem 400, der den Parameter nennt. `language` wird angenommen und ignoriert, `stream_format` ebenso, und der `Content-Type` ist bei jedem `response_format` `audio/mpeg`. Siehe [Modellseite zu Qwen3-TTS](../../models/qwen3-tts-customvoice/#fehlerantworten)
...
Gegenwärtig besteht **keine Möglichkeit, Bilder zu generieren**. Eine entsprechende API-Route existiert nicht.
