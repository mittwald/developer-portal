...
- Auf `/v1/audio/speech` antwortet eine ungültige `voice`, `language` oder `instructions` mit HTTP 400 und einem allgemeinen `Got bad request.`, das den Parameter nicht nennt. `response_format` und `speed` werden dagegen mit einer Meldung abgewiesen, die das Feld nennt. Siehe [Modellseite zu Qwen3-TTS](../../models/qwen3-tts-customvoice/#fehlerantworten)
