...
Zum Feld `allowed_openai_params` im Beispiel oben: `temperature` und `top_p` gehören zur Responses-API, `top_k` nicht. Parameter, die die Responses-API nicht kennt, werden entfernt, bevor die Anfrage geprüft wird. Ohne diese Freigabe antwortet die Anfrage also weiterhin mit `200`, und `top_k` bleibt wirkungslos. Dasselbe gilt für `presence_penalty`, `frequency_penalty` und `chat_template_kwargs`. Das Token-Budget heißt an diesem Endpunkt `max_output_tokens`; ein Feld `max_tokens` wird auf demselben Weg entfernt.

### Die Denkphase abschalten {#responses-thinking}

Am einfachsten über das Feld `reasoning` der Responses-API, das keine Freigabe braucht:
...
curl -i -X POST https://llm.aihosting.mittwald.de/v1/responses \
    ...
    -d '{
        ...
        "input": "Nenne die Hauptstadt von Portugal.",
        "reasoning": {"effort": "none"}
    }'
```

`"effort": "none"` schaltet die Denkphase bei Qwen3.5-122B-A10B-FP8, Qwen3.6-35B-A3B-FP8, Qwen3.8-27B-NVFP4 und Qwen3.5-0.8B ab. Jeder andere Wert lässt sie an, und genau so wird sie bei Qwen3.5-0.8B eingeschaltet, wo sie standardmäßig aus ist. gpt-oss-120b akzeptiert `"none"` nicht und antwortet mit `400`; dort ist `"low"` die kürzeste Denkkette.

Wer eine Anfrage von `/v1/chat/completions` übernimmt: `chat_template_kwargs` funktioniert hier ebenfalls, gehört aber nicht zur Responses-API und muss deshalb in `allowed_openai_params` genannt werden:
...
Ohne `allowed_openai_params` wird das Feld entfernt, die Anfrage antwortet mit `200` und das Modell denkt weiter. Ob eine der beiden Formen gewirkt hat, steht in der Antwort unter `usage.output_tokens_details.reasoning_tokens`: eine `0` bedeutet, dass keine Denkphase gelaufen ist.
...
Abhängig vom Embedding-Modell können weitere Parameter wie `dimensions` übermittelt werden. [Qwen3-Embedding-8B](../../models/qwen3-embedding-8b/) unterstützt `dimensions` mit einer modellspezifischen Auswahl an Werten. Die unterstützten Parameter und Werte findest du auf der jeweiligen Modellseite. Die Unterstützung ist eine Eigenschaft des einzelnen Modells und nicht des gesamten Endpunkts.
...
Der Antwortkörper ist die Audiodatei selbst, kein JSON. Schreib ihn direkt in eine Datei oder gib ihn an deinen Player weiter. Nur Fehler kommen als JSON zurück. Der `Content-Type` ist immer `audio/mpeg`, egal welches Format du angefordert hast. Benenn die Datei also nach deinem `response_format`.

Der Parameter `input` enthält den zu sprechenden Text und darf nicht leer sein. `model` wählt das TTS-Modell. `voice` ist Pflicht und muss einer von `aiden`, `dylan`, `eric`, `ono_anna`, `ryan`, `serena`, `sohee`, `uncle_fu`, `vivian` sein; ein unbekannter Name schlägt fehl.

Die übrigen Parameter sind optional:

- `response_format`: Einer von `wav` (Standard), `pcm`, `flac`, `mp3`, `opus`. `aac` wird **nicht** unterstützt
- `language`: Einer der dreizehn Sprachnamen, die das Modell spricht, zum Beispiel `German`. ISO-Codes wie `de` antworten mit HTTP 400, und ein gültiger Wert ändert wenig, siehe [Modellseite](../../models/qwen3-tts-customvoice/#sprache)
- `speed`: Sprechtempo zwischen 0,25 und 4,0, Standard 1,0
- `stream`: Auf `true` gesetzt bekommst du das Audio, während es noch entsteht. Das geht nur mit `response_format` `wav` und `pcm`
- `stream_format`: `audio` für rohe Audio-Chunks, `sse` für `speech.audio.delta`-Events im OpenAI-Format mit Base64-Audio

Ein ungültiger Wert für `voice`, `response_format`, `speed`, `language` oder `instructions` und ein leeres `input` antworten alle mit HTTP 400. Bei `response_format` und `speed` nennt die Meldung das Feld, bei `voice`, `language`, `instructions` und leerem `input` kommt `{"error":{"message":"Got bad request.","code":"400"}}` zurück.

Streaming lässt das Audio anfangen, bevor die Generierung durch ist: Der erste Chunk kommt in weniger als 150 ms, wo die gepufferte Antwort für denselben Text rund eine Sekunde braucht:
...
- Die beiden Audio-Endpunkte behandeln den Parameter `language` **unterschiedlich**. `/v1/audio/transcriptions` will ISO-639-1-Codes wie `de`, `/v1/audio/speech` weist die mit HTTP 400 ab und will den Sprachnamen, zum Beispiel `German`. Lies die Warnung auf der [Modellseite](../../models/qwen3-tts-customvoice/#sprache), bevor du gemeinsamen Code für beide schreibst
- Ohne Streaming braucht die Erzeugung etwa ein Fünftel der angeforderten Abspieldauer, und vorher kommt nichts an
- Voice-Cloning mit eigenem Referenz-Audio wird auf diesem Endpunkt nicht angeboten
- Teile lange Texte auf und fordere die Teile einzeln an. Ein ausgearbeitetes Beispiel steht im [Text-to-Speech-Guide](../../examples/text-to-speech/#lange-texte)
