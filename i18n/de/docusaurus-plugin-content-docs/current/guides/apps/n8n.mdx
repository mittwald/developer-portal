...
Du kannst n8n in deiner mittwald Hosting-Umgebung mit Containern bereitstellen. Es gibt verschiedene Ansätze:
...
:::caution
`NODES_EXCLUDE=[]` aktiviert **alle** Nodes für die Benutzung. Einige dieser Nodes können ein Sicherheitsrisiko darstellen. Zum Testen kann das sehr nützlich sein, für produktive Umgebungen wird empfohlen, eine genaue Liste unerwünschter Nodes zu pflegen.
:::

Sind alle Einstellungen vorbereitet, kann der Stack deployed werden:
...
Nach der Installation setzen wir das RAG-System in n8n auf und richten das System ein. Zunächst aktivieren wir einen n8n-Lizenzschlüssel und laden die Basisnodes zur Workflowerstellung herunter. Die benötigten Nodes liegen in `n8n-nodes-base` und können direkt aus der Applikation heraus heruntergeladen und installiert werden.
...
Neben dem bloßen Vorhandensein müssen einige der Nodes korrekt konfiguriert werden.
...
Lass den initialen Webhook im Test-Modus lauschen, dann kannst du z. B. per postman oder curl testen. Die genaue URL wird dir in der Trigger-Node angezeigt:
...
```shellsession
user@local $ scp my_documents/example_manual.txt <mittwald SSH URL n8n Container, siehe mStudio>:/home/node/rag_documents/
```
...
user@local $ scp -r python3_docs/*.txt <mittwald SSH URL n8n Container, siehe mStudio>:/home/node/rag_documents/
