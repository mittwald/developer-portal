...
Wenn dein Team bereits Build- und Runtime-Definitionen für Container pflegt, bleib bei `deploy-container-action`. Wenn du den kürzesten Weg vom Repository zum laufenden Service willst, nutze `zerodeploy-action`.

## Deploy mit `mittwald/zerodeploy-action` (Railpack) {#deploy-mit-zerodeploy-action-railpack}

[`mittwald/zerodeploy-action`](https://github.com/mittwald/zerodeploy-action) ist für Repositories gedacht, die noch kein Docker-Setup haben. Die Action installiert die mittwald CLI, installiert Railpack, baut aus deinem Repository und deployt in dein Zielprojekt.

### Benötigte Inputs {#benötigte-inputs-zerodeploy}
...
### Nützliche optionale Inputs {#nützliche-optionale-inputs-zerodeploy}
...
## Fortgeschrittene Muster für `mittwald/deploy-container-action` {#fortgeschrittene-muster}
