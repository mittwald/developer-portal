...
Verantwortung sind wichtig. Wir empfehlen dir, dich an unsere [Community](https://discord.gg/Fvtkwkwa8c) zu wenden, wenn du Fragen hast.
...
Mit anderen Worten: Du solltest deine mit KI entwickelte App mit minimalem Setup bereitstellen können,
idealerweise nur durch das Verbinden deines GitHub-Repositorys mit einer Bereitstellungsplattform, die
...
und du musst möglicherweise manuelle Anpassungen oder Konfigurationen vornehmen.
...
die es dir ermöglicht, deine Apps direkt von GitHub mit minimalem Setup bereitzustellen.
...
- Automatisiert über GitHub Actions (siehe [Deployment mit GitHub Actions](/docs/v2/guides/deployment/container-actions/), insbesondere die [`zerodeploy-action`](/docs/v2/guides/deployment/container-actions/#deploy-mit-zerodeploy-action))
...
- mittwald Skills installiert und für deinen Code-Agenten aktiviert
  (optional, aber empfohlen für Fehlerbehebung und Unterstützung während der Bereitstellung)
...
Die konkreten Befehle zum lokalen Ausführen deiner App hängen vom spezifischen Setup deiner App ab.
Häufige Informationsquellen dafür sind die README-Datei in deinem Repository,
der `package.json` Scripts-Bereich oder jegliche Dokumentation der KI-Entwicklungsplattform,
die du verwendet hast. Zusätzlich können Code-Agenten dabei helfen, die korrekten Befehle
zum lokalen Ausführen deiner App zu identifizieren, indem sie die Projektstruktur und Konfigurationsdateien analysieren.
...
:::tip Mit GitHub Actions automatisieren

Nachdem deine erste manuelle Bereitstellung erfolgreich war, richte automatisierte Deployments mit GitHub Actions ein. Die [`mittwald/zerodeploy-action`](/docs/v2/guides/deployment/container-actions/#deploy-mit-zerodeploy-action) bietet einen vollständigen Workflow, der deine App bei jedem Push automatisch baut und deployt.

Siehe die [vollständige Anleitung zum GitHub Actions Deployment](/docs/v2/guides/deployment/container-actions/) für schrittweise Anleitungen.

:::

Weiterführende Guides für tiefergehendes Setup:

- Für **automatisiertes Deployment mit GitHub Actions** beginne mit der [`zerodeploy-action`](/docs/v2/guides/deployment/container-actions/#deploy-mit-zerodeploy-action), die kein Dockerfile oder stack.yaml benötigt
- Für erweiterte Container-Deployments mit expliziten Stack-Definitionen siehe die [`deploy-container-action`](/docs/v2/guides/deployment/container-actions/#deploy-mit-deploy-container-action)
...
### Deployment von Replit zu mittwald {#deploying-replit-to-mittwald}

[Replits Git-Integration](https://docs.replit.com/references/workspace-tools/git-interface) exportiert Apps als Monorepo mit mehreren npm-Workspaces.
...
### Deployment von Lovable zu mittwald {#deploying-lovable-to-mittwald}

Mit Lovable erstellte Apps lassen sich als [Standard-Node.js-Projekt nach GitHub exportieren](https://docs.lovable.dev/integrations/github).
...
- **Community ist hier.** Unser [mittwald Discord](https://discord.gg/Fvtkwkwa8c)-Server ist ein großartiger Ort, um Fragen zu stellen, deine Projekte zu teilen,
