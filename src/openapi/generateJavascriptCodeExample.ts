...
function generateJavascriptCodeExample(method: string, url: string, spec: OpenAPIV3.OperationObject, baseURL: string) {
  ...
  for (const pathParam of pathParams) {
    requestObject[pathParam.name] = generateSchemaExample(pathParam.schema as OpenAPIV3.SchemaObject);
  }
  ...
  if (queryParams.length > 0) {
    ...
    for (const queryParam of queryParams) {
      requestObject.queryParameters[queryParam.name] = generateSchemaExample(queryParam.schema as OpenAPIV3.SchemaObject);
    }
  }
