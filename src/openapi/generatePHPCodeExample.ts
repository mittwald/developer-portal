...
function generatePHPCodeExample(method: string, url: string, spec: OpenAPIV3.OperationObject, baseURL: string) {
  ...
  const successfulStatus = Object.keys(spec.responses).find(status => status.startsWith("2")) ?? "200";

  const requestObject: any = {};
  const tag = tsTypeName(spec.tags[0]);
  const operationId = tsTypeName(spec.operationId.replace(new RegExp(`^${tag}-`, "i"), ""));

  const constructorParams: any = {};
  const constructorModifiers: any = {};
  const uses = [`\\Mittwald\\ApiClient\\Generated\\V2\\Clients\\${tag}\\${operationId}\\${operationId}Request`]
  ...
  for (const pathParam of pathParams) {
    constructorParams[pathParam.name] = JSON.stringify(generateSchemaExample(pathParam.schema as OpenAPIV3.SchemaObject));
  }
  ...
  if (bodySchema) {
    ...
    uses.push(`\\Mittwald\\ApiClient\\Generated\\V2\\Clients\\${tag}\\${operationId}\\${bodyClassName}`);
    constructorParams.body = "$body";
  }

  if (queryParams.length > 0) {
    for (const queryParam of queryParams) {
      constructorModifiers[queryParam.name] = JSON.stringify(generateSchemaExample(queryParam.schema as OpenAPIV3.SchemaObject));
    }
  }
  ...
  if (headers.length > 0) {
    ...
    for (const header of headers) {
      requestObject.headers[header.name] = generateSchemaExample(header.schema as OpenAPIV3.SchemaObject);
    }
  }

  const constructorParamsCode = Object.entries(constructorParams).
    map(([key, value]) => `\n  ${key}: ${value}`).
