...
// TODO: Examples for SDKs need to be disabled for v1 API
export function OperationUsage({ method, url, spec, baseURL, withJavascript = true, withPHP = true }: OperationUsageProps) {
  ...
  if (withJavascript) {
    items.push(<TabItem key="javascript" value="javascript" label="JavaScript SDK">
      <CodeBlock language="javascript">{generateJavascriptCodeExample(method, url, spec, baseURL)}</CodeBlock>
    </TabItem>);
  }

  if (withPHP) {
    items.push(<TabItem key="php" value="php" label="PHP SDK">
