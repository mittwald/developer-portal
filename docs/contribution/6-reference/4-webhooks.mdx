...
- `context`: Der extension context kind the extension instance was added to. Currently one of `customer` and `project`.
...
[DSA signature](https://www.rfc-editor.org/rfc/rfc6605.html) encoded in base 64.

You can use common cryptographic libraries in your preferred programming language to verify the signature.
The coplete and unprocessed request body has to be verified using the `verify` method.
