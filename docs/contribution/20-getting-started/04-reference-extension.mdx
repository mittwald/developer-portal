...
Die Reference Extension verwendet **[PostgreSQL](https://www.postgresql.org/)** für die Datenpersistierung.
Sie bietet außerdem eine `docker-compose.yml`, mit der eine Datenbank lokal hochgefahren werden kann.

Um die Datenbank hochzufahren und das Datenbank-Schema anzuwenden, führe folgendes aus:

```bash
# PostgreSQL Container starten (läuft auf Port 5433)
...
Nachdem alle Voraussetzungen erfüllt sind, kann der Development Server gestartet werden:

```bash
pnpm run dev
```

Der Server startet standardmäßig auf **Port 3000** und ist unter `http://localhost:3000` erreichbar.

**Was passiert beim Start:**

- Vite Development Server wird gestartet
- TanStack Start lädt die Server-seitigen Routes
- Datenbank-Verbindung wird hergestellt
- Webhook-Endpoints werden unter `/api/webhooks.mittwald` verfügbar
