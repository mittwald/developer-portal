...
An Extension needs permissions to access the REST API of the mStudio and perform [**domain actions**](../how-to/ensure-authorization).
...
The Context has to be set before Extension Instances can be created.
...
Once an Extension has been verified, a contributor can request changes to the Extension’s scopes and webhook URLs. 
The request will be revied by a mittwald employee within **2–3 business days**.  
 
If the request is approved, the Extension continues to function for all existing users. 
Newly added scopes remain unavailable until each user explicitly accepts the updated permission set. 
Users who have already installed the Extension will receive a prompt to grant the new scopes. 
Only after a user accepts the changes can the Extension access the newly added capabilities.

### Assets

Assets are media files (photos or videos) that are displayed in the marketplace listing to showcase the features of the Extension, UI, or demo footage.  
