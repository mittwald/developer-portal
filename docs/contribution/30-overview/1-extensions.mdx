...
Due to api stability guarantees the `state` field is deprecated.
The fields `disabled`, `published`, `verifcatonRequested`, `deprecated`, `functional` and `verified` of the `OwnExtension`,
as found at <OperationLink operation="extension-get-own-extension" />; display the state of the Extension.

- `disbled`: The extension is disabled. No new **ExtensionInstances** can be created.
- `published`: Everyone can now add the extension to their own **Context**.
- `verifcatonRequested`: mittwald is verifying the **Extension**.
- `deprecated`: The **Extension** is deprecated by the **Contributor**.
- `functional`: The **Extension** operates as expected.
- `verified`: The Extension has been verified by mittwald and can now be published.
...
---
stateDiagram
A: ExtensionInstanceAddedToContext
B: ExtensionInstanceActivated
C: ExtensionInstanceDeactivated
D: ExtensionInstanceRemoved
A --> B
C --> D
B --> D: Removed by end user
C --> B: Extension chechout perfomed
B --> C: Extension became costly
```
