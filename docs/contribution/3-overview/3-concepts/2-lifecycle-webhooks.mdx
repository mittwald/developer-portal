...
Only the `ExtensionInstanceRemovedFromContext` webhook call is executed and retried until the extension processes the call successfully or the extension is removed from the marketplace.
