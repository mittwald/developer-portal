...
The prompting for login information by the external application is not an accepted method.
...
For more information on using the received access token, see [Nutzung von Access Tokens zur Authentifizierung](../../../reference/api#using-access-tokens-to-authenticate).
