...
An Extension acts on behalf of an mStudio User, if it uses the
[session-token](../../overview/concepts/frontend-development#authentication-and-session-handling-using-session-tokens),
the[access token retrieval key](../../overview/concepts/authentication#access-token-retrieval-key)
