...
- **Extension ID**:  
  You can copy the Extension ID in the **Entwicklung** tab of your Extension in mStudio.

- **Scopes**:  
  In the **Permissions** section of mStudio, you will find the selected scopes.  
  For every selected scope, add an entry to the `scopes` field in your request body using the format:\
	`"area:(read|write|delete)"`. 

![Extension permissions section](../../../resources/contributing/extensions/extension-permissions.jpg)

:::Tip
When creating an Extension Instance via the mStudio you dont have to worry about setting the correct context and
selecting the correct scopes.
:::
