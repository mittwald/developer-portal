...
`opencode run` needs to approve the file write before it happens. Pass `--auto` to
auto-approve permissions that are not explicitly denied, and `--standalone` to run
against a private server instead of the shared background service.
