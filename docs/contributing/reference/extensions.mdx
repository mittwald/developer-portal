...
Extensions werden derzeit von mittwald in das mStudio eingepflegt.
Dazu werden einige Informationen benötigt, die nachfolgend beschrieben werden.
...
<SchemaWithExample withRawJSONSchema schema={{
  type: "object",
  required: ["id", "contributorId", "name", "description", "support", "state", "extensionContext", "requiredScopes", "externalComponents"],
  properties: {
    id: {
      type: "string",
      format: "uuid",
    ...
    description: {
      type: "string",
      description: "Kurzbeschreibung der Extension. Sollte möglichst kurz und prägnant die Extension erklären",
    ...
    externalComponents: {
      ...
      properties: {
        backend: {
          type: "object",
          required: ["extensionAddedToContext", "extensionInstanceUpdated", "extensionInstanceSecretRotated", "extensionInstanceRemovedFromContext"],
          properties: {
            ...
            extensionInstanceSecretRotated: {
              type: "string",
              format: "uri",
              description: "URL, die aufgerufen wird, wenn ein Secret einer Extension Instance rotiert wird. Kann Platzhalter enthalten",
