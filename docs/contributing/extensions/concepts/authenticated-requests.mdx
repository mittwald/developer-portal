...
- Wichtig: Wir haben bedacht, dass das Token sehr kurzlebig und nur einmalig gültig ist
- mithilfe des User-IDs und des ATRK kann man sich über "die Route" ein Access Token holen
- Access Token repräsentiert den mStudio-User, eingeschränkt durch die Scopes der Extension

## Extension Secret

Um selbständig agieren zu können, erhält die Extension ein eigenes Secret. Dieses wird der externen Anwendung mittels Lifecycle Events übermittelt. In Kombination mit der `Extension Instance ID` kann die externe Anwendung dieses verwenden, um ein Access Token für das mStudio zu beziehen. Dieses Access Token kann anschließend verwendet werden, um Domain Actions auszuführen.
