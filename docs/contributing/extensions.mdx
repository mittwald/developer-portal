...
![External Application](../../resources/contributing/extensions/external-application.png)
...
After successfully authenticating the user, the user can perform Domain Actions against the public API. Note that currently this is only possible through an external backend because of CORS restrictions. In future, possibilities of automatically accept Cross Origins of Extensions External Applications will be evaluated.

### ... as not registered user in context of an External Application

Performing a Domain Action in context of an external application as a not registered user is the key use case for Extensions with external applications, establishing a separate user management or trying to provide features that are not directly coupled to manual interaction of a specific mStudio User. The external application has to provide an endpoint for incoming Lifecycle Webhook calls to obtain ExtensionInstanceID and Extension Secret pairs (see above) that can be used to gain an Extension Access Token. This Extension Access Token can be used to perform Domain Actions against the public API.

![Extension Secret Authentication](../../resources/contributing/extensions/extension-secret-authentication.png)
...
This flow is interesting applications that are a hybrid of the previously mentioned approaches (e.g. to implement an admin panel for registered mStudio users and a user panel for not registered endcustomers). Further it is relevant for Frontend Fragments (see below).

![Access Token Retrieval Key in Context of Extension](../../resources/contributing/extensions/access-token-retrieval-key-extension.png)

![Frontend Fragments](../../resources/contributing/extensions/frontend-fragment.png)
