...
**In summary:**  
Custom API clients aren't just about personal or company preferences—they’re a key enabler for integration, compliance, security, and developer productivity. By designing a client tailored to your specific requirements, you ensure your applications remain robust, maintainable, and future-proof.

# Preparations

Thorough preparation sets the stage for a smooth and efficient custom API client development process. With the right groundwork, it’s often possible to generate most of your client automatically, with minimal manual intervention. Below are key preparation areas to consider before you start:
...
- **Adjust generator settings:**  
  Configure type mappings, serialization options, HTTP client selection, package naming, and code style to match your project’s conventions.
- **Supporting files:**  
...
> Let your manual efforts go into improving automation, testing, and pipeline robustness—rather than into laborious edits of generated code.
...
**Requirements:**
- Python Installation (version 3.8+ recommended)
...
> **TODO:** If you want to skip straight to automation and maintenance, jump ahead to the [maintenance & pipeline section](#maintenance-pipeline).

---

## Creating a Python mittwald API Client

Before automating the workflow, start by manually generating a client on your console. First, create your project folder, set up a Python virtual environment, and install the client generator:
...
::note
`(venv)` indicates your virtual environment is active. This alters your `PATH`, so `python` points to the local interpreter. Activation for bash/zsh is shown; for other shells (PowerShell, fish, etc.), see `venv/bin/`. Deactivate with `deactivate`. On Windows, activation is `venv\Scripts\activate`.
::

> **Python Version:**  
> Python 3.8+ is recommended for compatibility with typical generator output.
...
::warning
Never commit `.env` or other secret files to version control. Use `.gitignore` files to early hide such files from version control.
::

::note
For advanced or high-performance use cases, consider using the async API available in modern clients. Start synchronous for simplicity; move to async as needed.
::
...
::note
Generated package names (like `mittwald-api-client`) may collide with existing packages in PyPI. If needed, adjust the generator config or package name before publishing to avoid conflicts.
::
...
Follow instructions in the generated README for publishing. Example workflow for test PyPI:
    ...
    steps:
      ...
      - name: Install dependencies
        run: |
          ...
          pip install openapi-python-client poetry

      - name: Download OpenAPI specification
        run: curl --location --fail --silent 'https://api.mittwald.de/v2/openapi.json' > openapi.json

      - name: Regenerate Client
        run: openapi-python-client generate --path ./openapi.json --force
      ...
      - name: Run Tests
        run: |
          # Add your real test commands here, e.g. pytest tests/
          echo "Run your tests!"
...
- **Secure secrets:**  
  Store your PyPI token in your GitHub repository’s `Secrets` settings as `PYPI_TOKEN`; never hard-code it.
- **Testing:**  
