...
2. After downloading the backup, you can extract the database contents from the downloaded archive:
    ...
    :::note
    Note that in the archive files, the database ID is spelled differently. Whereas it usually follows the format `mysql_XXXXXX`, in the archive, it is spelled with dashes instead of underscores, like `mysql-XXXXXX`.
    :::
