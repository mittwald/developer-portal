...
```shell
wp config set --type=constant WP_REDIS_HOST HOST
wp config set --type=constant WP_REDIS_PORT PORT
wp config set --type=constant WP_REDIS_DATABASE 0
wp config set --type=constant WP_REDIS_SCHEME tcp
```

- use the `hostname` and `port` specified for the database as values for `WP_REDIS_HOST` and `WP_REDIS_PORT` (see [`GET https://api.mittwald.de/v2/redis-databases/{id}`](https://developer.mittwald.de/reference/v2#tag/Database/operation/database-get-redis-database))
