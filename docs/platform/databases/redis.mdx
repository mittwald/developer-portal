...
<Tabs groupId="access-mode">
  ...
  <TabItem value="cli" label="CLI">
    ...
    ```shellsession
    user@local $ mw database redis create --description "My Redis database" --version 7.2
    ```
...
```shellsession
user@ssh $ wp plugin install redis-cache --activate
```

and set the required configuration constants:

```shellsession
user@ssh $ wp config set --type=constant WP_REDIS_HOST HOST
user@ssh $ wp config set --type=constant WP_REDIS_PORT PORT
user@ssh $ wp config set --type=constant WP_REDIS_DATABASE 0
user@ssh $ wp config set --type=constant WP_REDIS_SCHEME tcp
```

- use the `hostname` and `port` specified for the database as values for `WP_REDIS_HOST` and `WP_REDIS_PORT` (see <OperationLink operation="database-get-redis-database" />)
