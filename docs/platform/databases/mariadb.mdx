...
### Using Terraform (Recommended) {#using-terraform}
...
### Using the mStudio UI {#using-mstudio-ui}
...
### Using CLI with `mw container run` {#using-cli-container-run}
...
### Using CLI with `mw stack deploy` {#using-cli-stack-deploy}
...
#### Option 1: Direct access via container shell {#direct-access-container-shell}
...
#### Option 2: Port forwarding for local access {#port-forwarding-local-access}
