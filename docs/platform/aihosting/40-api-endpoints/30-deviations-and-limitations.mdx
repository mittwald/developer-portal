...
- On `/v1/audio/speech`, an invalid parameter value returns HTTP 500 with a generic message instead of a 400 naming the parameter, `language` is accepted and ignored, `stream_format` is ignored, and `Content-Type` is `audio/mpeg` for every `response_format`. See the [Qwen3-TTS model page](../../models/qwen3-tts-customvoice/#error-responses)
