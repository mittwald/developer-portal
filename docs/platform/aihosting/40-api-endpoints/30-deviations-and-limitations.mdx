...
- On `/v1/audio/speech`, an invalid `voice`, `language` or `instructions` returns HTTP 400 with a generic `Got bad request.` that does not name the parameter, while `response_format` and `speed` are rejected with a message that does. See the [Qwen3-TTS model page](../../models/qwen3-tts-customvoice/#error-responses)
