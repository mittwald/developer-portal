...
GLM-OCR is a purpose-built document understanding model. Unlike general vision models, it is optimised exclusively for reading text — making it the right choice whenever you need to digitise PDFs, parse scanned invoices, or extract fields from forms at high accuracy and low cost.

:::tip When to use GLM-OCR
Pick GLM-OCR when your goal is **text extraction** from any document format (PDF, DOCX, PPTX, XLSX, HTML, images). For tasks that combine understanding and conversation (e.g. "summarise and explain this contract"), a general vision model such as [Qwen3.5-122B-A10B-FP8](/docs/v2/platform/aihosting/models/qwen3-5-122b-a10b-fp8) may be a better fit. Of course, you can pre-process your documents with GLM-OCR and then forward the text to another LLM.

See the [GLM-OCR model reference](/docs/v2/platform/aihosting/models/glm-ocr) for supported formats, request limits, and all inference parameters.
:::
...
GLM-OCR supports three output modes, controlled entirely through your prompt:

| Mode | When to use | Example prompt |
| ---- | ----------- | -------------- |
| **Plain text** | Full-text search, logging, simple digitisation | `"Extract all text from this document."` |
| **Markdown** | Preserving document structure (headings, lists, tables) for display or RAG | `"Extract the text and format it as Markdown. Use # for headings and - for lists."` |
| **JSON** | Extracting specific named fields from forms, invoices, certificates | `"Extract these fields and return them as JSON: …"` |

:::note[Output behaviour]
**JSON mode always wraps output in markdown code fences** (` ```json … ``` `) — you must strip them before calling `json.loads()`. See the [KIE example](#extracting-structured-data-from-invoices) below for the stripping pattern.

**Markdown mode needs explicit formatting instructions.** A generic "convert to Markdown" prompt may not produce heading syntax. Always specify `# for main headings, ## for subheadings, - for lists`.

**HTML source → HTML tables.** When the input is an HTML file containing a `<table>`, the model tends to reproduce it as HTML even if you ask for markdown tables. Use a PDF or image source when you need clean markdown table output.
:::
...
:::note[Data handling and privacy]
**No document data is written to disk.** The proxy renders your document to PNG images entirely in RAM and forwards them directly to the model — it retains no data after each response. External URLs embedded in HTML, CSS, and SVG are explicitly blocked to prevent SSRF attacks. Document content is processed entirely within the mittwald infrastructure and is not sent to any third party.
:::
...
:::note[Page limit]
Up to **30 pages** are processed per request. The API returns HTTP 413 for larger documents — see [Batch-processing large PDFs](#batch-processing-large-pdfs) below.
:::
...
:::note[XLSX output]
For Excel files, the model includes the **worksheet name** as the first line of output, followed by the cell content. Strip or handle that first line if you are post-processing the result programmatically.

Office files must be valid Open XML documents. Corrupted or hand-crafted ZIP structures return HTTP 400.
:::
