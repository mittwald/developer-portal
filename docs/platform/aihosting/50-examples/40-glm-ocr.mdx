...
Supported formats, limits, and inference parameters are in the [GLM-OCR model reference](/docs/v2/platform/aihosting/models/glm-ocr).
:::
...
GLM-OCR has three output modes, all controlled through the prompt:
...
:::note[Data handling]
**No document data is written to disk.** The proxy converts documents to PNG images in memory and passes them straight to the model — nothing is cached or stored between requests. External URLs inside HTML, CSS, or SVG files are blocked to prevent SSRF. All processing stays on mittwald servers.
:::
