...
## Optional: reranking with Qwen3-VL-Reranker-2B {#reranking}
...
`Qwen3-VL-Reranker-2B` is served via the `/v1/rerank` endpoint. Drop it in between the `retrieve()` and `answer()` steps in the [pipeline above](#full-pipeline):
