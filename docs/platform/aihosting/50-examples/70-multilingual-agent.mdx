...
description: Build a tool-calling agent that handles 100+ languages with Qwen3.5-122B-A10B-FP8
title: Multilingual tool-calling agent
---

The tool-calling agent pattern in this guide works with any chat model that supports function calling. The example uses [Qwen3.5-122B-A10B-FP8](/docs/v2/platform/aihosting/models/qwen3-5-122b-a10b-fp8), but you can substitute any of the following depending on cost and quality requirements:
...
def run_agent(user_message: str, system: str | None = None) -> str:
    ...
    while True:
        response = client.chat.completions.create(
            ...
            # Thinking mode is on by default for this model — disable it here
            # to keep tool-call latency down across the agent loop.
        )
