...
For vision tasks, lower temperature (`temperature=0.1`) is recommended when accurate image description or text extraction is required. Always resize images to a maximum of 1024 px on the longest edge before encoding - large images significantly increase time to first token (TTFT). See the [Python examples](../../examples/python/#vision-encoding) or [JavaScript examples](../../examples/javascript/#vision-encoding) for a ready-to-use helper.
