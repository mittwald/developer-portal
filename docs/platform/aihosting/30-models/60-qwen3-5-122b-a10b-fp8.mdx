...
Sending this to `/v1/responses` instead of `/v1/chat/completions`? `chat_template_kwargs` is not part of the Responses API and is dropped there unless it is named in `allowed_openai_params` — see [Turning the thinking phase off](../../api-endpoints/supported-endpoints#responses-thinking), which also covers the endpoint's own `reasoning.effort` field.
...
:::tip Document Q&A pipeline
For a ready-to-run example using Qwen3.5-122B-A10B-FP8 as the answering model in a document RAG pipeline — combined with [GLM-OCR](/docs/v2/platform/aihosting/models/glm-ocr) and [Qwen3-Embedding-8B](/docs/v2/platform/aihosting/models/qwen3-embedding-8b) — see the [GLM-OCR guide](../../examples/glm-ocr/).
:::
