...
Download the official VCL configuration for your application (see [Application-Specific VCL Configurations](#application-specific-vcl-configurations) below) and modify it to add Host header rewriting in the `vcl_backend_fetch` subroutine (see the CLI section above for the specific changes needed).
