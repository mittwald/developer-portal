...
Alternatively, start with a local project and then [deploy][deployment] it into the application directory.
...
### Running multiple worker instances (with PM2) {#running-multiple-worker-instances}
...
### Running multiple worker instances (with Supervisord) {#running-multiple-worker-instances-supervisord}

[Supervisord](https://supervisord.org/index.html) is another popular process manager that can be used to run multiple worker instances within the same worker app.

To use Supervisord, add Python as a dependency to your PHP worker app:

```shellsession
user@local $ mw app dependency update <app-id> --set python=~3.13
```

Then, install Supervisord via pip:

```shellsession
user@ssh $ pip install supervisor
```

In your application directory, create a configuration file `supervisord.conf`:
...
Finally, assert that the `supervisord` command is used as the entrypoint for your PHP worker app:

```shellsession
user@local $ mw app update <app-id> --entrypoint "supervisord"
```

After this, you can also use the `supervisorctl` command to manage your worker processes:
