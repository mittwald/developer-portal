...
The registry set up in this chapter can be used as private registry, so make sure to make registry reachable, e.g. by creating a subdomain in your project pointing towoards custom registry.
...
After volume creation, we can ship our prepared auth folder:
...
<Tabs groupId="access-mode">
  ...
  <TabItem value="cli" label="CLI">
    ...
    Make sure to prepare and ship `auth` folder as described earlier.
...
Recreate the container with volumes and environment variables set.
...
Your push **must** be `rejected` due to missing basic auth credentials. Now log in:
...
Depending on your project structure and use case, one generic docker registry in a separate project **or** one registry per project can be created and used. 
