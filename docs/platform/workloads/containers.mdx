...
<Tabs groupId="access-mode">
  ...
  <TabItem value="api" label="API">
    ...
    The `cpus` field specifies the maximum number of CPUs the container can use (as a string, e.g., `"0.5"` for half a CPU). The `memory` field specifies the maximum amount of memory the container can use (e.g., `"512m"` for 512 megabytes, `"1gb"` for 1 gigabyte).
