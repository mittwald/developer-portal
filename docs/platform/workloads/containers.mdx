...
For mutable tags (such as `latest`), this is often combined with a recurring update schedule so the stack regularly pulls and applies the newest image version.
...
```shellsession
user@local $ mw stack set-update-schedule st-abcde "0 3 * * *"
```

Use the stack short ID as `<stack-id>` (for example `st-abcde`).
