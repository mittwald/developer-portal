...
In order to easily configure authentication for the registry, we will be using the [`mittwald/registry` image](https://hub.docker.com/repository/docker/mittwald/registry/general) instead of the regular `registry` image from Docker Hub. This image is based on the official `registry` image, but includes additional features and configuration options for authentication.
...
<Tabs groupId="access-mode">
  <TabItem value="gui" label="mStudio UI">
    ...
    4. Configure the container as needed, use `mittwald/registry:3` image.
    5. Set up environment variables to your liking; check the guide above for details.
    6. To configure authentication (for private images), additionally supply a `REGISTRY_USER` and a `REGISTRY_PASSWORD` environment variable with secure credentials of your choice.
  ...
  <TabItem value="cli" label="CLI">
    ...
    user@local $ mw container run \
      ...
      --env "REGISTRY_PASSWORD=<your-secure-password>" \
      --create-volumes \
      mittwald/registry:3
    ```

    Replace `<your-username>` and `<your-secure-password>` with secure credentials of your choice. Note that passwords passed via command line may be stored in shell history.
...
in your project pointing towards custom registry.
...
:::warning
Without additional configuration this registry is publicly available without authentication.
If login via credentials is desired, remember to supply the `REGISTRY_USER` and `REGISTRY_PASSWORD` environment variables with secure credentials of your choice.
:::
...
If you have configured authentication via `REGISTRY_USER` and `REGISTRY_PASSWORD`, you need to log in before pushing images:
...
### Updating a mutable tag {#updating-a-mutable-tag}
...
For mutable tags (such as `latest`), this is often combined with a recurring update schedule (see [Scheduling recurring container updates](#update-schedule)) so the stack regularly pulls and applies the newest image version.
...
For databases running in containers, see the guide on [backing up database containers](/docs/v2/guides/operations/database-container-backups), which shows how to schedule consistent SQL dumps into a volume so that the project backup picks them up.

## Network connectivity between containers and apps {#stack-network-connectivity}
...
## Scheduling recurring container updates {#update-schedule}

If your deployment strategy uses mutable tags, you can configure a schedule for your stack updates. This allows the stack to regularly pull newer image versions without running the update command manually each time.

You can configure the update schedule in the mStudio UI, via the CLI, or via the API with <OperationLink operation="container-set-stack-update-schedule" apiVersion="v2" />.

Use the `mw stack set-update-schedule` command:

```shellsession
user@local $ mw stack set-update-schedule <stack-id> <schedule> [--timezone=<timezone>]
```

The `<schedule>` value is a cron expression. For example, to run the update daily at 03:00 UTC:

```shellsession
user@local $ mw stack set-update-schedule 11111111-2222-3333-4444-555555555555 "0 3 * * *"
```

Use the full stack ID (UUID format) as `<stack-id>`.

Use this especially when your containers reference mutable tags such as `latest`, so new image versions are applied automatically according to your schedule.
