...
<Tabs groupId="environment-variables">
  <TabItem value="below-20-6" label="< Node.js 20.6">
    Du kannst Umgebungsvariablen in folgenden Dateien definieren. Sie sind dann automatisch in deiner App verfügbar:

    - `/files/.mwsoftware/node`
    - `/files/.mwsoftware/node/.config/node/.env`
    - `/etc/node/.env`

  </TabItem>
  <TabItem value="from-20-6" label=">= Node.js 20.6">
    Ab Node.js 20.6 steht ein (experimentelles) Flag `--env-fileconfig` zur Verfügung (vgl. [Node.js-Dokumentation](https://nodejs.org/dist/latest-v20.x/docs/api/cli.html#--env-fileconfig)). Dieses kann beliebig oft angegeben werden, um .env-Dateien zu laden, deren Umgebungsvariablen dann in der App verfügbar sind.
