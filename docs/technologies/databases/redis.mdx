...
For all versions up to TYPO3 11.5 you create a file named `AdditionalConfiguration.php` in the `typo3conf` folder. For TYPO3 version 12.4 and higher you create a file named additional.php in the folders typo3conf/system (for symlink installations) or config/system (for composer installations).
Please add the following in each case, if the file already exists, you can omit `<?php`.
