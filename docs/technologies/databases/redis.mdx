...
In the WordPress backend, go to _Plugins_ and click on "Add New". Search for "Redis Object Cache" and install the plugin that appears as the first result. After installation, add the following entry to `wp-config.php`:
...
Create a file named `AdditionalConfiguration.php` in the `typo3conf` folder and add the following. If the file already exists, you can omit `<?php`.
