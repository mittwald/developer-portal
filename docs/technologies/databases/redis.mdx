...
We recommend the plugin
**[Redis Object Cache](https://wordpress.org/plugins/redis-cache/)**.

Install the plugin "Redis Object Cache"
...
- use the `hostname` and `port` specified for the database as values for
  `WP_REDIS_PATH` and `WP_REDIS_PORT` (see
...
If you don’t like to use the WP-CLI, you can also install the plugin via the
WordPress backend. After installation and activation, add the following entries
to `wp-config.php`:
