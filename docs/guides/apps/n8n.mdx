...
You can deploy n8n in your mittwald hosting environment as a container. There are several main approaches:
...
2. Enter the image `n8nio/n8n:stable`. You can use the default entrypoint and command.
...
Some mittwald AI Hosting models — for example [Qwen3.8-27B-NVFP4](../../../platform/aihosting/models/qwen3-8-27b-nvfp4), [Qwen3.6-35B-A3B-FP8](../../../platform/aihosting/models/qwen3-6-35b-a3b-fp8) and [Qwen3.5-122B-A10B-FP8](../../../platform/aihosting/models/qwen3-5-122b-a10b-fp8) — ship with "thinking mode" enabled by default. Disabling (or, for other models, enabling) it requires sending `enable_thinking` nested inside `chat_template_kwargs` via the `extra_body` field of the chat completion request. See the respective model page for the exact request format.
...
n8n can be used to build [Retrieval-Augmented Generation (RAG)](https://en.wikipedia.org/wiki/Retrieval-augmented_generation) systems of various forms. The following example shows one possible setup, but due to the large n8n ecosystem, it is only one of many approaches. This example is intended to serve as a starting point for building more complex and especially production-ready systems.
...
- **Agent**: `Ministral-3-14B-Instruct-2512`
...
```shellsession
user@local $ mw stack deploy -c path/to/n8n/docker-compose.yml --env-file path/to/n8n/example.env
```
...
After basic operation is confirmed, the RAG system can be populated with documents. In the example, documents are stored, processed, and made accessible to the AI agent in the filesystem. In practice, you may also want to consider other sources in addition to the filesystem, or configure the filesystem against a service, or an Internet share (e.g., Google Share).
...
```shellsession
user@local $ mw container ssh <CONTAINER-ID>
user@n8n $ mkdir -p /home/node/rag_documents
```

Activate the monitoring node in n8n, deliver documents:

```shellsession
user@local $ scp my_documents/example_manual.txt <mittwald SSH URL n8n Container, see mStudio>:/home/node/rag_documents/
```
...
user@local $ scp -r python3_docs/*.txt <mittwald SSH URL n8n Container, see mStudio>:/home/node/rag_documents/
...
- Split database setup: Sepearate n8n data and RAG vector databases, allowing more precise ressource allocation and focussed backups
