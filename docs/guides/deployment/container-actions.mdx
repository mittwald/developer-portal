...
If your team already maintains container build and runtime definitions, stay with `deploy-container-action`. If you want the fastest path from repository to running service, use `zerodeploy-action`.
...
### Useful optional inputs {#useful-optional-inputs-zerodeploy}

- **`repo-subpath`**; deploy only a subdirectory (for monorepos)
- **`node-version`**; Node.js runtime used inside the action (default: `24`)
- **`mittwald-cli-version`**; CLI version constraint (default: `^1.17.0`)
