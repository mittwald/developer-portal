...
responsibility matter, and we recommend connecting with our [community](https://discord.gg/Fvtkwkwa8c) if you have questions.
...
In other words: You should be able to deploy your AI-developed app with minimal setup,
ideally just by connecting your GitHub repository to a deployment platform that supports
...
and you may need to be prepared to make some manual adjustments or configurations in certain cases.
...
allowing you to deploy your apps directly from GitHub with minimal setup.

Currently, there are two main paths for zero-conf deployment of AI-developed apps:
- Manually via mittwald CLI
- Automated via GitHub Actions (see [deploying with GitHub Actions](/docs/v2/guides/deployment/container-actions/), specifically the [`zerodeploy-action`](/docs/v2/guides/deployment/container-actions/#deploy-with-zerodeploy-action))
...
- mittwald skills installed and enabled for your code agent of choice
  (optional but recommended for troubleshooting and guidance during deployment)
...
The concrete commands to run your app locally will depend on the specific setup of your app.
Common sources of information for this are the README file in your repository,
the `package.json` scripts section, or any documentation provided by the AI development
platform you used. Additionally, code agents can assist in identifying the correct commands
to run your app locally by analyzing the project structure and configuration files.
...
:::note
Note that the command used here is implemented in the `experimental` namespace,
which means that it is still in development and may be subject to change.
While the underlying code is stable, the command interface and behavior may evolve
as we gather feedback and iterate on the feature.
We recommend checking the latest documentation for any updates or changes to
...
Test your app in a browser, deployment command will create a subdomain and link to container target.
If you encounter any issues, check the logs and configuration to troubleshoot.
Logs can be found in the container section in mStudio or via the CLI with container ID:

```shellsession
user@local $ mw container logs c-XXXXXX
```

:::tip Automate with GitHub Actions

After your first manual deployment succeeds, set up automated deployments with GitHub Actions. The [`mittwald/zerodeploy-action`](/docs/v2/guides/deployment/container-actions/#deploy-with-zerodeploy-action) provides a complete workflow that automatically builds and deploys your app on every push.

See the [full GitHub Actions deployment guide](/docs/v2/guides/deployment/container-actions/) for step-by-step instructions.

:::

Related guides for deeper setup:

- For **automated deployment with GitHub Actions**, start with the [`zerodeploy-action`](/docs/v2/guides/deployment/container-actions/#deploy-with-zerodeploy-action) which requires no Dockerfile or stack.yaml
- For advanced container deployments with explicit stack definitions, see the [`deploy-container-action`](/docs/v2/guides/deployment/container-actions/#deploy-with-deploy-container-action)
...
### Deploying from Replit to mittwald {#deploying-replit-to-mittwald}

[Replit's Git integration](https://docs.replit.com/references/workspace-tools/git-interface) ships exported apps as a monorepo with multiple npm workspaces.
This means that if you export your app to GitHub, you can deploy it to mittwald
with some adjustments, most importantly picking the right workspace to build and serve.

In order to override Replit's default build and start commands, you can add
following variables during build time, e.g. to pick the `hello-world` workspace
in the example monorepo. From GitHub action workflow file:
...
[Replit Git interface documentation](https://docs.replit.com/references/workspace-tools/git-interface)

### Deploying from Lovable to mittwald {#deploying-lovable-to-mittwald}

Apps created with Lovable can be [exported to GitHub](https://docs.lovable.dev/integrations/github) as a standard Node.js project.
In order to deploy such app to mittwald hosting, we must ensure that the exported app
has a valid `package.json` file with the necessary build and start scripts defined.

In addition, Lovable apps might fail in deployment due to unresolved dependencies or
missing configuration files. To resolve these issues, it might be helpful to even
remove additional package managers like `bun` and stick to `npm` or `pnpm` for better
compatibility:
...
Further reading on underlying package manager detection heuristics and configuration
options can be found in the [railpack documentation](https://railpack.com/languages/node).
...
up to date. These practices are straightforward and make your app more reliable. Both CLI command and
GitHub action support environment variables, so you can keep secrets out of your code and manage them securely.
- **Test early.** Deploy locally first, share with friends, and gather feedback before going live.
- **Community is here.** Our [mittwald Discord](https://discord.gg/Fvtkwkwa8c) server is a great place to ask questions, share what you're building,
