...
> **Note:** Do not commit your `.env` file to source control; add it to `.gitignore`.
...
user@local $ python tools/overlay_generator.py openapi.json openapi_filter_overlay.json

# apply overlay create "openapi_from_overlay.json"
user@local $ oas-patch overlay openapi.json openapi_filter_overlay.json -o openapi_from_overlay.json
```
