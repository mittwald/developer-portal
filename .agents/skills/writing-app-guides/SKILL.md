---
name: writing-app-guides
description: Conventions for app and database deployment guides in docs/guides/apps and docs/platform/databases (and their German counterparts). Use when authoring or reviewing a "Running <App>" or migration guide — covers front matter, document structure, the required order of deployment methods (Terraform, mStudio UI, mw container run, mw stack deploy), mStudio UI walkthrough format, environment variable blocks, Docker image references, troubleshooting sections and container port constraints.
---

# Writing app and database deployment guides

## When to use this skill

These guidelines apply specifically to app and database deployment guides:

- `docs/guides/apps/*.md`, `docs/guides/apps/*.mdx`
- `docs/platform/databases/*.md`, `docs/platform/databases/*.mdx`
- `i18n/de/docusaurus-plugin-content-docs/**/guides/apps/*.md`,
  `i18n/de/docusaurus-plugin-content-docs/**/guides/apps/*.mdx`
- `i18n/de/docusaurus-plugin-content-docs/**/platform/databases/*.md`,
  `i18n/de/docusaurus-plugin-content-docs/**/platform/databases/*.mdx`

Follow the general `writing-documentation` skill as well.
...
- **sidebar_label**: Use the application name (e.g. "Directus", "Collabora") or a short action
  phrase for migration guides (e.g. "Migrate Node.js App to Container")
- **description**: Start with "Learn how to..." or "Step-by-step guide for..." followed by the main
  action
- Do NOT include a `title` field in the front matter; use a markdown H1 heading instead
...
Use "Running [App Name]" for standard deployment guides, or a descriptive title for
migration/specialized guides:
...
Include a blockquote from the official documentation when available:
...
Alternatively, use "Requirements" as the heading with a numbered list for more complex setups.

### 4. Container deployment section

Use "How do I start the container?" as the main section heading. Document deployment methods in this
order:
...
- Use a `:::caution` callout to make users aware of default credentials that _MUST_ be replaced with
...
Always reference Docker images with a link to the Docker Hub page and the full image name as used in
commands:
...
For some applications, there might be a mittwald Terraform module available. These can usually be
found on GitHub, following the format `mittwald/terraform-mittwald-[app]` (e.g.
`mittwald/terraform-mittwald-directus`), and on the Terraform Registry under
`mittwald/[app]/mittwald`.
...
- Exposing a port only makes that port accessible from _within_ the hosting environment (meaning,
  containers or managed apps running within the same project). To access the port from outside the
  hosting environment, a user can either use the `mw container port-forward` command to forward the
  port to their local machine, or connect a domain to the port. The latter only supports HTTP ports.
- Exposing a port _within_ the hosting environment is NOT a security issue. The platform has network
  policies that ensure that no unauthorized access is possible.

## Linking conventions

- Link to platform documentation using absolute paths: `/docs/v2/platform/workloads/containers`
- Link to the CLI reference: `/docs/v2/cli/reference/container`
