---
name: writing-api-guides
description: Conventions for API how-to guides in docs/api/howtos (and their German counterparts under i18n/de/docusaurus-plugin-content-docs/**/api/howtos). Use when authoring or reviewing a guide that explains how to do something via the mStudio API — covers front matter, prerequisites and workflow structure, the OperationLink/OperationExample/OperationHint/SchemaFromSpec components, placeholder conventions, and documenting request and response properties.
---
...
These guidelines apply specifically to API guides in the `docs/api/howtos` directory:

- `docs/api/howtos/**/*.md`, `docs/api/howtos/**/*.mdx`
- `i18n/de/docusaurus-plugin-content-docs/**/api/howtos/**/*.md`,
  `i18n/de/docusaurus-plugin-content-docs/**/api/howtos/**/*.mdx`
...
- **sidebar_label**: Use the present participle form, e.g. "Creating a project", "Managing DNS
  records"
- **title**: Full descriptive title ending with "using the API" or "via the API", e.g. "Creating a
  project using the API"
- **description**: Start with "Learn how to..." or "This article describes...", followed by the
  action and "via the mStudio API"
...
Most API guides should start with a "Prerequisites" section that lists what the reader needs before
following the guide:
...
Use `<OperationLink />` to reference API operations. Never hardcode URLs to API reference
documentation.
...
Use `<OperationHint />` at the end of sections to provide a quick reference to the full API
documentation:
...
When listing multiple related API operations, use a bulleted list with `<!-- prettier-ignore -->` to
...
- Link to platform documentation when explaining concepts:
